...
if "page" not in st.session_state:
    st.session_state.page = "main"
# 'face_photo_object'도 초기에 None으로 설정하는 것이 안전합니다.
...
# 2. 퍼스널 컬러 분석 페이지
    ...
    # ✨ 1. 카메라 켜고 끄기 버튼
    if st.button("사진 촬영 📸", use_container_width=True):
        st.session_state.show_camera = not st.session_state.show_camera

    uploaded_photo = st.file_uploader("또는 얼굴 사진 업로드 📂", type=["jpg", "jpeg", "png"])

    # ✨ 2. show_camera 상태가 True일 때만 카메라 위젯 표시
    if st.session_state.show_camera:
        camera_photo = st.camera_input("카메라")
        # 사진이 촬영되면, 그 사진을 state에 저장하고 카메라는 끔
        if camera_photo:
            st.session_state.face_photo_object = camera_photo
            st.session_state.show_camera = False
            st.rerun() # 즉시 새로고침하여 카메라 끄고 사진만 표시

    # 파일이 업로드되면 state에 저장
    if uploaded_photo:
        st.session_state.face_photo_object = uploaded_photo

    # ✨ 3. state에 저장된 사진이 있을 때만 이미지와 분석 버튼 표시
        ...
        with col2:
            if st.button("이미지 삭제", use_container_width=True):
                ...
                if "analyzed_color" in st.session_state:
                    del st.session_state.analyzed_color
                st.rerun()
    ...
    if "analyzed_color" in st.session_state and st.session_state.face_photo_object:
        if st.button(f"'{st.session_state.analyzed_color}' 결과를 내 정보에 적용하기"):
            ...
            if 'user_info' in st.session_state:
                st.session_state.user_info['피부_톤'] = st.session_state.analyzed_color
