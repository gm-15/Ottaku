...
def recommend_size(height, weight, gender):
    """키, 몸무게, 성별을 기반으로 일반적인 사이즈를 추천하는 함수"""
    bmi = weight / ((height / 100) ** 2)

    if gender == "남자":
        if height < 170: top_size = "M (95)"
        elif 170 <= height < 180: top_size = "L (100)" if bmi < 25 else "XL (105)"
        else: top_size = "XL (105) 이상"

        if weight < 65: bottom_size = "28-30 inch"
        elif 65 <= weight < 75: bottom_size = "31-33 inch"
        elif 75 <= weight < 85: bottom_size = "34-36 inch"
        else: bottom_size = "37 inch 이상"
    else:  # 여자
        if height < 160: top_size = "S (44-55)"
        elif 160 <= height < 168: top_size = "M (55-66)" if bmi < 22 else "L (66-77)"
        else: top_size = "L (66-77) 이상"

        if weight < 50: bottom_size = "25-26 inch (S)"
        elif 50 <= weight < 58: bottom_size = "27-28 inch (M)"
        elif 58 <= weight < 68: bottom_size = "29-30 inch (L)"
        else: bottom_size = "31 inch (XL) 이상"
...
def analyze_clothing_image(uploaded_image):
    ...
    try:
        ...
        if match: return json.loads(match.group(0))
            ...
            st.error("AI 응답에서 JSON을 찾을 수 없습니다."); st.code(response.text)
            return None
    except Exception as e:
        st.error(f"이미지 분석 중 오류 발생: {e}"); return None
...
def get_cody_recommendation_with_image(user_info, clothing_info, situation):
    ...
    try:
        ...
        st.error(f"코디 추천 중 오류 발생: {e}"); return None, None, None
...
def analyze_personal_color(face_image):
    ...
    * 전체적인 조화: [전체적인 이미지와 색의 조화에 대한 분석]
    ...
    try:
        ...
        st.error(f"퍼스널 컬러 분석 중 오류 발생: {e}"); return None
...
if st.session_state.page == "main":
    ...
    with tab1:
        st.subheader("성별"); 성별 = st.radio("성별", ["남자", "여자"], horizontal=True, key="gender_radio")
        st.subheader("체형"); 키 = st.number_input("키(cm)", 100, 250, st.session_state.get("키", 170), 1)
        ...
        st.subheader("패션"); st.image("img/img.png")
        fashion_styles_options = ["캐주얼", "시크", "시티보이", "클래식", "에스닉", "걸리시", "고프코어", "미니멀", "프레피", "리조트", "레트로", "로맨틱", "스포티", "스트리트", "워크웨어"]
    ...
    with tab_size:
        ...
        if 'user_info' in st.session_state:
            gender, height, weight = st.session_state.user_info['성별'], st.session_state.user_info['키'], st.session_state.user_info['몸무게']
            ...
            if st.button("내 사이즈 추천받기", use_container_width=True):
                ...
                st.success("사이즈 추천 완료!"); st.markdown(f"### 추천 상의: **{sizes['상의']}**"); st.markdown(f"### 추천 하의: **{sizes['하의']}**")
                st.info("※ 위 추천은 일반적인 체형 기준이며, 브랜드나 핏에 따라 다를 수 있습니다.")
        else: st.warning("먼저 '나의 맞춤 정보' 탭에서 정보를 저장해주세요.")
    with tab2:
        ...
        if uploaded_file:
            if "last_uploaded_filename" not in st.session_state or st.session_state.last_uploaded_filename != uploaded_file.name:
                st.session_state.analysis_result = None; st.session_state.recommendation_output = None
                st.session_state.last_uploaded_filename = uploaded_file.name
            col1, col2 = st.columns(2)
            with col1: st.image(uploaded_file, caption="업로드된 옷 이미지", use_column_width=True)
            with col2:
                if st.button("AI로 옷 분석 시작하기", use_container_width=True):
                    if 'user_info' not in st.session_state: st.error("먼저 '나의 맞춤 정보' 탭에서 정보를 저장해주세요!")
                    else:
                        with st.spinner("AI가 이미지를 분석하고 있습니다... 🧠"):
                            analysis_result = analyze_clothing_image(uploaded_file)
                            if analysis_result:
                                st.session_state.analysis_result = analysis_result
                                st.session_state.user_activity_log.append(analysis_result); st.rerun()
                if st.session_state.get("analysis_result"):
                    result = st.session_state.analysis_result
                    st.success("분석 완료!"); st.subheader("✅ AI 분석 결과")
                    st.write(f"**의류 종류**: {result.get('item_type', 'N/A')}"); st.write(f"**카테고리**: {result.get('category', 'N/A')}")
                    st.write(f"**색상**: {result.get('color', 'N/A')}"); st.write(f"**패턴**: {result.get('pattern', 'N/A')}")
                    tags = result.get('style_tags', []); st.write(f"**스타일 태그**: {', '.join(tags) if tags else 'N/A'}")
                    if st.button("👚 옷장에 추가하기", use_container_width=True):
                        closet_item = {"image": uploaded_file.getvalue(), "name": uploaded_file.name, "analysis": result}
                        st.session_state.my_closet.append(closet_item); st.success(f"'{uploaded_file.name}'을(를) 옷장에 추가했습니다!")
        ...
        if 'analysis_result' in st.session_state and 'user_info' in st.session_state:
            ...
            if st.button("AI 코디 추천 및 이미지 생성", use_container_width=True):
                ...
                with st.spinner("AI 스타일리스트가 코디를 만들고 이미지를 생성합니다... ✨"):
                    recommendation_text, image_prompts, search_keywords = get_cody_recommendation_with_image(st.session_state.user_info, st.session_state.analysis_result, situation)
                    if recommendation_text and image_prompts:
                        image_urls = [generate_image_with_dalle(prompt) for prompt in image_prompts]
                        st.session_state.recommendation_output = {"text": recommendation_text, "keywords": search_keywords, "image_urls": image_urls}
                    else: st.session_state.recommendation_output = None; st.error("코디 추천에 실패했습니다.")
            if st.session_state.get("recommendation_output"):
                output = st.session_state.recommendation_output
                st.subheader("AI 스타일리스트의 추천"); st.markdown(output["text"], unsafe_allow_html=True)
                    ...
                    for i, url in enumerate(output["image_urls"]):
                        with cols[i]:
                            if url:
                                ...
                                if url in st.session_state.saved_images: st.success("✅ 저장됨")
                                else:
                                    if st.button("💾 이 코디 저장하기", key=save_key, use_container_width=True):
                                        success, filepath = save_image_from_url("saved_outfits", url)
                                        if success: st.session_state.saved_images.append(url); st.success(f"저장 완료!"); st.rerun()
                                        else: st.error("저장에 실패했습니다.")
                            else: st.warning(f"추천 코디 {i + 1} 이미지 생성 실패.")
        else: st.warning("먼저 '옷 분석하기'를 완료해주세요.")

# 3.2. 오늘의 날씨 페이지 (신규 추가)
        ...
        with st.spinner('날씨 데이터를 가져오는 중입니다...'):
            ...
            if weather_json:
                ...
                if not df.empty:
                    ...
                    col1.metric("하늘 상태", sky); col2.metric("강수 형태", pty); col3.metric("강수 확률", f"{pop}%")
                    col4, col5 = st.columns(2)
                    col4.metric("풍속", f"{wsd} m/s"); col5.metric("습도", f"{reh}%")
...
# 3.3. 나의 옷장 페이지
    ...
    if not st.session_state.my_closet: st.info("아직 옷장에 저장된 옷이 없습니다.")
    else:
        cols = st.columns(4)
        for i, item in enumerate(st.session_state.my_closet):
            with cols[i % 4]:
                ...
                if st.button("삭제", key=f"delete_closet_{i}", use_container_width=True):
                    st.session_state.my_closet.pop(i); st.rerun()
    st.write("---")
    st.subheader("저장된 추천 코디")
    if not st.session_state.saved_images: st.info("아직 저장된 추천 코디가 없습니다.")
    else:
        cols = st.columns(4)
        for i, url in enumerate(st.session_state.saved_images):
            with cols[i % 4]:
                ...
                if st.button("삭제", key=f"delete_saved_{i}", use_container_width=True):
                    st.session_state.saved_images.pop(i); st.rerun()
...
# 3.5. 패션 데이터 분석 페이지
    ...
    if not st.session_state.user_activity_log: st.info("아직 분석된 옷 데이터가 없습니다.")
    else:
        ...
        with col1:
            if not tag_counts.empty:
                fig_style = px.bar(tag_counts, y=tag_counts.index, x=tag_counts.values, orientation='h', title="나의 스타일 선호도 분석", labels={'y': '스타일', 'x': '분석 횟수'}, color=tag_counts.values, color_continuous_scale='viridis')
                st.plotly_chart(fig_style, use_container_width=True)
            else: st.info("스타일 데이터가 부족합니다.")
        with col2:
            if not color_counts.empty:
                fig_color = px.pie(values=color_counts.values, names=color_counts.index, title="분석된 옷 색상 분포", color_discrete_sequence=px.colors.qualitative.Pastel)
                st.plotly_chart(fig_color, use_container_width=True)
            else: st.info("색상 데이터가 부족합니다.")
    st.markdown("### 🌍 최신 패션 트렌드 분석 (Google Trends)")
    with st.spinner("Google Trends에서 최신 데이터를 가져오는 중..."):
        trends_df = get_google_trends_data()
        if not trends_df.empty:
            st.line_chart(trends_df); st.caption("지난 1년간의 주요 패션 키워드에 대한 관심도 변화입니다.")
        else: st.warning("트렌드 데이터를 가져오는 데 실패했습니다.")
